...
import os
from dialog.TagSample.model_2 import TagSimple
...
parser.add_argument("-config",
                    default="/home/panxie/Documents/myGAN/multi-response/config.yml",
                    type=str)
parser.add_argument("-vocab",
                    default="/home/panxie/Documents/myGAN/multi-response/data/weibo/weibo.vocab.txt",
                    type=str)
parser.add_argument('-train_data',
                    default="/home/panxie/Documents/myGAN/multi-response/data/weibo/sampler_data.tsv",
                    type=str)
parser.add_argument('-train_ids_file',
                    default="/home/panxie/Documents/myGAN/tf_multi_response/data/weibo/sampler_data_idx.csv",
                    type=str)
...
parser.add_argument("-log_dir",
                    default="/home/panxie/Documents/myGAN/tf_multi_response/out/log_dir",
                    type=str)
parser.add_argument("-model_dir",
                    default="/home/panxie/Documents/myGAN/tf_multi_response/out/model_save",
                    type=str)
...
# 定义训练集和验证集
my_dataset = get_dataset(args.vocab, args.train_data, vocab_size=config.Misc.tag_vocab_size)
def train_input_fn():
    return my_dataset._train_input_fn(args.train_data, args.train_ids_file, batch_size=config.TagSampler.batch_size)

def eval_input_fn():
    return my_dataset._eval_input_fn(args.eval_data, args.eval_ids_file)
...
eval_spec = estimator.EvalSpec(
    ...
    steps=100)
EstimatorConfig = estimator.RunConfig(
    tf_random_seed=100,

)

def train_and_eval():
    esti_tagsimple = estimator.Estimator(
        model_fn=model_fn,
        model_dir=os.path.join(args.model_dir, "train"),
        params=EstimatorConfig)

    estimator.train_and_evaluate(esti_tagsimple, train_spec, eval_spec)
